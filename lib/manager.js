...
function isOriginAllowed(origin, allowedOrigin, request) {
  if (typeof allowedOrigin === 'function') {
    return allowedOrigin(origin, request);
  } else if (Array.isArray(allowedOrigin)) {
    for (var i = 0; i < allowedOrigin.length; ++i) {
      if (isOriginAllowed(origin, allowedOrigin[i], request)) {
        return true;
      }
    }
    return false;
...
Manager.prototype.isOriginAllowed = function (origin, request) {
  return isOriginAllowed(origin, this.get('origins'), request)
};
